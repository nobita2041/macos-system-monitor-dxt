...
export class ProcessAnalyzer {
  ...
  async killProcess(pid, force = false) {
    try {
      // First, get process info to ensure it's safe to kill
      const processes = await si.processes();
      ...
      const enhanced = this.enhanceProcessInfo(targetProcess);
      
      if (!enhanced.safe_to_kill && !force) {
        return {
          success: false,
          error: `Process ${enhanced.name} is not safe to kill. Use force=true if you're sure.`
        };
      }
      ...
      const verification = await si.processes();
      const stillExists = verification.list.find(p => p.pid === pid);
      
      if (stillExists) {
        return {
          success: false,
          error: `Process ${enhanced.name} (${pid}) could not be terminated`
        };
      }
      
      return {
        success: true,
        message: `Process ${enhanced.name} (${pid}) terminated successfully`
      };
